...
function key(type,keyName,code=keyName){window.dispatchEvent(new KeyboardEvent(type,{key:keyName,code,bubbles:true,cancelable:true}))}
function tapKey(keyName,code=keyName){key('keydown',keyName,code);setTimeout(()=>key('keyup',keyName,code),45)}
function mouse(canvas,type,x,y,button=0,buttons=0){canvas.dispatchEvent(new MouseEvent(type,{clientX:x,clientY:y,button,buttons,bubbles:true,cancelable:true}))}
function blurEditor(){const a=document.activeElement;if(a?.matches?.('input,textarea,select,[contenteditable="true"]'))a.blur()}
function init(){
  ...
  const updateStick=e=>{blurEditor();const r=stick.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dx=e.clientX-cx,dy=e.clientY-cy,max=r.width*.32,m=Math.hypot(dx,dy)||1,scale=Math.min(1,max/m),x=dx*scale,y=dy*scale;knob.style.transform=`translate(${x}px,${y}px)`;const nx=dx/Math.max(1,r.width*.24),ny=dy/Math.max(1,r.height*.24),next=new Set();if(ny<-.35)next.add('w');if(ny>.35)next.add('s');if(nx<-.35)next.add('a');if(nx>.35)next.add('d');syncKeys(next)};
  const stopStick=()=>{stickId=null;knob.style.transform='translate(0,0)';syncKeys(new Set())};
  stick.addEventListener('pointerdown',e=>{stickId=e.pointerId;stick.setPointerCapture?.(e.pointerId);updateStick(e);e.preventDefault()});stick.addEventListener('pointermove',e=>{if(e.pointerId===stickId){updateStick(e);e.preventDefault()}});stick.addEventListener('pointerup',stopStick);stick.addEventListener('pointercancel',stopStick);
  let aimX=.72,aimY=.5,touchId=null;
  const aim=e=>{const r=canvas.getBoundingClientRect();aimX=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width));aimY=Math.max(0,Math.min(1,(e.clientY-r.top)/r.height));mouse(canvas,'mousemove',e.clientX,e.clientY,0,e.buttons||0)};
  const canvasDown=e=>{if(e.pointerType!=='touch')return;blurEditor();touchId=e.pointerId;canvas.setPointerCapture?.(e.pointerId);aim(e);mouse(canvas,'mousedown',e.clientX,e.clientY,0,1);e.preventDefault()},canvasMove=e=>{if(e.pointerType==='touch'&&e.pointerId===touchId){aim(e);e.preventDefault()}},canvasUp=e=>{if(e.pointerType!=='touch'||e.pointerId!==touchId)return;aim(e);mouse(canvas,'mouseup',e.clientX,e.clientY,0,0);touchId=null;e.preventDefault()};
  ...
  const action=layer.querySelector('[data-mobile="action"]');let held=false;const point=()=>{const r=canvas.getBoundingClientRect();return{x:r.left+r.width*aimX,y:r.top+r.height*aimY}};
  const actionDown=e=>{if(held)return;blurEditor();held=true;const p=point();mouse(canvas,'mousemove',p.x,p.y);mouse(canvas,'mousedown',p.x,p.y,0,1);e.preventDefault()},actionUp=e=>{if(!held)return;held=false;const p=point();mouse(canvas,'mouseup',p.x,p.y,0,0);e.preventDefault()};
  ...
  const hotbar=gameRoot.querySelector('#hotbar');hotbar?.addEventListener('click',e=>{const slot=e.target.closest('.slot');if(!slot)return;const slots=[...hotbar.querySelectorAll('.slot')],i=slots.indexOf(slot);if(i>=0&&i<9)tapKey(String(i+1),`Digit${i+1}`)},true);
  const onResize=()=>shell.classList.toggle('mobile-portrait',window.innerHeight>window.innerWidth);window.addEventListener('resize',onResize);onResize();
  cleanup=()=>{syncKeys(new Set());window.removeEventListener('resize',onResize);canvas.removeEventListener('pointerdown',canvasDown);canvas.removeEventListener('pointermove',canvasMove);canvas.removeEventListener('pointerup',canvasUp);canvas.removeEventListener('pointercancel',canvasUp);layer.remove();for(const p of panels)p.classList.remove('mobile-open');shell.classList.remove('mobile-mode','mobile-portrait');cleanup=null};
}
