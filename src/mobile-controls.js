...
const labels={
  en:{action:'ACT',dash:'DASH',interact:'USE',inventory:'BAG',chat:'CHAT',build:'BUILD',squad:'SQUAD',close:'Close',rotate:'Landscape is recommended for easier control.'},
  ko:{action:'행동',dash:'대시',interact:'사용',inventory:'가방',chat:'채팅',build:'건설',squad:'분대',close:'닫기',rotate:'조작하기 편하도록 가로 화면을 권장합니다.'},
  ja:{action:'行動',dash:'ダッシュ',interact:'使用',inventory:'バッグ',chat:'チャット',build:'建築',squad:'分隊',close:'閉じる',rotate:'操作しやすい横画面を推奨します。'},
  zh:{action:'行动',dash:'冲刺',interact:'使用',inventory:'背包',chat:'聊天',build:'建造',squad:'小队',close:'关闭',rotate:'建议横屏游玩以便操作。'}
};
...
function init(){
  ...
  const canvasDown=e=>{if(e.pointerType!=='touch')return;blurEditor();canvasTouchId=e.pointerId;canvas.setPointerCapture?.(e.pointerId);aim(e);mouse(canvas,'mousedown',e.clientX,e.clientY,1);e.preventDefault()};
  ...
  const actionDown=e=>{if(actionHeld)return;blurEditor();actionHeld=true;const p=actionPoint();mouse(canvas,'mousemove',p.x,p.y,0);mouse(canvas,'mousedown',p.x,p.y,1);e.preventDefault()};
  ...
  for(const p of panels){
    if(!p.querySelector('.mobile-panel-close')){const close=document.createElement('button');close.type='button';close.className='btn mobile-panel-close';close.textContent=`× ${t('close')}`;close.onclick=()=>p.classList.remove('mobile-open');p.prepend(close)}
  }
  const togglePanel=index=>{const target=panels[index],opening=target&&!target.classList.contains('mobile-open');panels.forEach(p=>p.classList.remove('mobile-open'));if(opening)target.classList.add('mobile-open')};
