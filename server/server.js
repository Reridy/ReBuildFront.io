...
const checkPassword=(password,stored)=>{try{const[salt,hex]=stored.split(':');const expected=Buffer.from(hex,'hex');const actual=scryptSync(password,salt,64);return expected.length===actual.length&&timingSafeEqual(expected,actual)}catch{return false}};
...
function auth(req){const raw=req.headers.authorization||'';const token=raw.startsWith('Bearer ')?raw.slice(7):'';const session=sessions.get(token);if(!session)return null;const user=readUsers().find(u=>u.id===session.userId);return user?{token,user}:null}
...
app.post('/api/auth/signup',(req,res)=>{
  ...
  const language=supportedLang.has(req.body?.language)?req.body.language:'en';
  if(username.length<3||password.length<8)return res.status(400).json({error:'INVALID_CREDENTIALS'});
  const users=readUsers();
  if(users.some(u=>u.username.toLowerCase()===username.toLowerCase()))return res.status(409).json({error:'USERNAME_TAKEN'});
  const user={id:randomUUID(),username,passwordHash:hashPassword(password),language,providers:[],createdAt:Date.now(),profile:{customTemplates:[],settings:{}}};
  users.push(user);saveUsers(users);res.status(201).json({token:issueSession(user),user:publicUser(user)});
});
app.post('/api/auth/login',(req,res)=>{
  const username=cleanName(req.body?.username);
  const password=String(req.body?.password||'');
  const user=readUsers().find(u=>u.username.toLowerCase()===username.toLowerCase());
  if(!user||!checkPassword(password,user.passwordHash))return res.status(401).json({error:'INVALID_LOGIN'});
...
app.put('/api/account/preferences',(req,res)=>{const a=auth(req);if(!a)return res.status(401).json({error:'UNAUTHORIZED'});const users=readUsers();const u=users.find(x=>x.id===a.user.id);if(supportedLang.has(req.body?.language))u.language=req.body.language;u.profile=u.profile||{};u.profile.settings={...(u.profile.settings||{}),...(req.body?.settings||{})};if(Array.isArray(req.body?.customTemplates))u.profile.customTemplates=req.body.customTemplates.slice(0,12);saveUsers(users);res.json({user:publicUser(u)})});
...
app.post('/api/rooms',(req,res)=>{
  const roomName=cleanText(req.body?.name,40)||'Room';
  const description=cleanText(req.body?.description,160);
  const mapId=req.body?.map==='test'?'test':'test';
  const difficulty=['easy','normal','hard'].includes(req.body?.difficulty)?req.body.difficulty:'normal';
  const maxPlayers=Math.max(1,Math.min(MAX_PLAYERS,Number(req.body?.maxPlayers)||4));
  const language=supportedLang.has(req.body?.language)?req.body.language:'en';
  const id=randomUUID().slice(0,8);
  const room={id,name:roomName,description,map:mapId,difficulty,maxPlayers,language,hostId:null,hostName:null,players:new Map(),started:false,world:{heartHp:1000,wave:0,phase:'prep'},updatedAt:Date.now()};
  rooms.set(id,room);res.status(201).json(roomSummary(room));
});
...
io.on('connection',socket=>{
  ...
  socket.on('ready',value=>{const r=rooms.get(socket.data.roomId);const p=r?.players.get(socket.id);if(!r||!p||p.id===r.hostId)return;p.ready=Boolean(value);broadcastRoom(r)});
  ...
  socket.on('input',input=>{const r=rooms.get(socket.data.roomId);const p=r?.players.get(socket.id);if(!p)return;p.input={x:Math.max(-1,Math.min(1,Number(input?.x)||0)),y:Math.max(-1,Math.min(1,Number(input?.y)||0))}});
  socket.on('chat',payload=>{const r=rooms.get(socket.data.roomId);const sender=r?.players.get(socket.id);if(!r||!sender)return;const text=cleanText(payload?.text);if(!text)return;if(payload?.type==='whisper'){const target=findByName(r,cleanName(payload?.target));if(!target)return socket.emit('chat-error',{code:'PLAYER_NOT_FOUND'});const message={id:randomUUID(),type:'whisper',sender:sender.name,target:target.name,text,createdAt:Date.now()};socket.emit('chat-message',message);if(target.id!==socket.id)io.to(target.id).emit('chat-message',message);return}io.to(r.id).emit('chat-message',{id:randomUUID(),type:'global',sender:sender.name,text,createdAt:Date.now()})});
